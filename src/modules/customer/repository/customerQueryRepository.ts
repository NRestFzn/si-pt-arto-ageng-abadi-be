...
import { CustomerQueryFilterDto } from '../dto'

export class CustomerQueryRepository extends BaseQueryRequest {
  ...
  constructor(req: Request) {
    ...
    const query = req.query as unknown as CustomerQueryFilterDto
    ...
    const npwpValue = query.npwp as unknown
    this.npwp = typeof npwpValue === 'string' ? npwpValue : undefined
  ...
  public queryFilter(): FindOptions {
    const whereCondition: WhereOptions<CustomerQueryFilterDto>[] = []
    ...
    if (this.npwp !== undefined && this.npwp !== null && this.npwp !== '') {
      whereCondition.push({ npwp: { [Op.like]: `%${this.npwp}%` } })
    }
