...
import { ExpenseQueryFilterDto } from '../dto'

export class ExpenseQueryRepository extends BaseQueryRequest {
  ...
  constructor(req: Request) {
    ...
    const query = req.query as unknown as ExpenseQueryFilterDto
    ...
    const accountValue = query.account as unknown
    this.account = typeof accountValue === 'string' ? accountValue : undefined
    ...
    const bank_accountValue = query.bank_account as unknown
    this.bank_account = typeof bank_accountValue === 'string' ? bank_accountValue : undefined
  }
  ...
  public queryFilter(): FindOptions {
    const whereCondition: WhereOptions<ExpenseQueryFilterDto>[] = []
    ...
    if (this.account !== undefined && this.account !== null && this.account !== '') {
      whereCondition.push({ account: { [Op.like]: `%${this.account}%` } })
    }
    ...
    if (this.bank_account !== undefined && this.bank_account !== null && this.bank_account !== '') {
      whereCondition.push({ bank_account: { [Op.like]: `%${this.bank_account}%` } })
    }
